...
- `load_conversation` 用于恢复知识库工作台：按 `conversation_id` 读取指定会话，或按 `knowledge_base_id` 读取最近更新的会话，并返回持久化的 sources 与精简 tool trace。
...
知识库前端按 library 保存最近选中的 `knowledge_base_id`；重新进入页面后先恢复知识库，再从此接口恢复对话、sources 和 tool trace。
